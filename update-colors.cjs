...
for (const filePath of filePaths) {
  ...
  content = content.replace(/#0A0A0A/g, '#1A1A1A');
  
  // Teal to Burgundy
  content = content.replace(/#00C9A7/g, '#B03030');
  
  // Off-white/Gray panels to Cream/Pink
  content = content.replace(/#F4F6F8/g, '#FCF0F0');
  ...
  // FF4757 (Red) to B03030 (Burgundy) - because previously red was high risk, but now it's brand red
  content = content.replace(/#FF4757/g, '#B03030');

  // white background to FCF5F5
  content = content.replace(/bg-white/g, 'bg-[#FCF5F5]');
